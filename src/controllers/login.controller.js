...
login.resetPassword = async (req, res, next) => {
  try {
    ...
    if (checklink && Object.keys(checklink)?.length) {
      ...
      const updatePass = await loginService.updateUserPassword(checklink?.user_id, encryptedPassword);
...
login.forgotPassword = async (req, res, next) => {
  try {
      ...
      const uuid = uuidv4();
      const passTrigger = await loginService.createPasswordChangeRequest(userExist?.id, uuid);
...
login.adminRefreshToken = async (req, res, next) => {
  try {
      ...
      const existQuery = await loginService.findAdminUserByEmailAndId(refreshTokenUser?.name, refreshTokenUser?.id);
      ...
      const getId = await loginService.findRefreshTokensByAdminUserId(existQuery?.dataValues?.id);
